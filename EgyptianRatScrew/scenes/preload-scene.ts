...
export class PreloadScene extends Phaser.Scene {
  ...
  private setupLoadingEvents(): void {
    ...
    // Handle loading errors - but continue anyway
  ...
  private loadAssets(): void {
    // Set a flag to track if we have assets to load
    let hasAssets = false;

    // Try to load assets, but don't fail if they don't exist
    try {
      // Only load if the file actually exists - for now we'll skip this
    ...
    // Optional: Load other assets if they exist - commented out for now
